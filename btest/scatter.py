...
"""
Grand scatter plot of btest associations
===============================================
...
from . import viz, config
from viz import scatter_matrix
...
def load_table(args):
    try:
        associations = blockplot.load_associations(str(args.input) + '/associations.txt', largest=None, strongest=None,
                                                   orderby='similarity')
    ...
    if args.outfile:
        viz.scatter_matrix(df_all, x_size=len(row_items), filename=args.outfile)
        viz.scatter_matrix(df_all_rank, x_size=len(row_items), filename='ranked_' + args.outfile)
    else:
        viz.scatter_matrix(df_all_rank, x_size=len(row_items),
                           filename='Scatter_association_' + str(args.association_number) + '_rank.pdf')
        viz.scatter_matrix(df_all, x_size=len(row_items),
                           filename='Scatter_association_' + str(args.association_number) + '.pdf')
