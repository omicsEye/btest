"""
Central namespace for plotting capacities in btest,
...
# matplotlib.use( "Agg" )
def plot_box(data, alpha=.1, figure_name='btest_Evaluation', xlabel='Methods', ylabel=None, labels=None):
    import pylab as pl
    import numpy as np
    # multiple box plots on one figure

    pl.figure("btest False Discovery Rate Controlling", dpi=300, figsize=(10, 5))
    if ylabel == "FDR":
    ...
    pl.scatter(np.repeat(np.arange(len(data)) + 1, len(data[0])), [item for sublist in data for item in sublist],
               marker='+', alpha=1)
...
def plot_roc(roc_info=None, title=None, figure_name='roc_plot_btest', ax=None):
    """
    =======================================
...
def heatmap2(pArray1, pArray2=None, xlabels=None, ylabels=None, filename='./hierarchical_heatmap2', metric="nmi",
             method="single", colLable=True, rowLabel=True, color_bar=False, scale='sqrt'):
    if len(pArray2) == 0:
        pArray2 = pArray1
        ylabels = xlabels
    pMetric = stats.c_hash_metric[metric]
...
def heatmap(data_table, D=[], xlabels_order=[], xlabels=None, ylabels=[],
    ...
    if stats.c_hash_association_method_discretize[config.similarity_method]:
...
def grouped_boxplots2(data, title, threshold_line=[], xlabels=[], ylabel="Recall/FDR", xlable_rotation=10,
                      file_name="Grouped_Recall_FDR", ax=None):
    ...
    # plt.xlim([-0.05, 1.15])
    # plt.ylim([-0.05, 1.15])
    groups = grouped_boxplots(data, ax, patch_artist=True, max_width=0.5, threshold_line=threshold_line, notch=0,
                              sym='+', vert=1, whis=1)
...
def grouped_boxplots(data_groups, ax, max_width=0.95, pad=0.05, threshold_line=[.1], show_points=False, **kwargs):
    ...
    for i, group in enumerate(data_groups, start=1):
        ...
        if i % 2 == 0:
            # print pos
            ax.bar(np.mean(pos), 1, zorder=0, color=".985", width=(width + 2 * pad) * 2,
                   edgecolor="none")  # np.mean(pos)-(width+2*pad)
            # '''width * len(group) + pad * (len(group) - 1)-width/2 -pad'''
            # plt.setp(artist, color ='red')
        # artist.patch.set(facecolor='0.1')
        else:
            ax.bar(np.mean(pos), 1, zorder=0, color="0.955", width=(width + 2 * pad) * 2,
                   edgecolor="none")  # np.mean(pos)-(width+2*pad)
        ax.get_xaxis().set_tick_params(which='both', labelsize=8, top='off', direction='out')
        ax.get_yaxis().set_tick_params(which='both', labelsize=8, right='off', direction='out')
        ...
        if len(threshold_line) == 1:
            ...
            ax.axhline(y=threshold_line[int((i - 1) / 2)], xmin=float((i - 1)) / len(data_groups),
                       xmax=float(i) / len(data_groups), linewidth=.5, color='r', alpha=1)
        if show_points:
...
def scatter_matrix(df, x_size=0, filename=None, ):
    ...
    for i in range(len(axs[:, 0])):
        for j in range(len(axs[-1, :])):
            ...
            if i > j:
                axs[i, j].scatter(df[df.columns[j]], df[df.columns[i]], c=color, s=10, alpha=.5, linewidths=.25,
                                  edgecolors='black')
                # import ggplot as gg
    ...
    for ax in axs[:, 0]:  # the left boundary
        ...
        ax.set_ylabel(wrap(ax.get_ylabel()), fontsize=7, rotation=0, va='center', ha='left',
                      labelpad=len(ax.get_ylabel()) + 25)  # fontweight='bold',
    ...
    for ax in axs[-1, :]:  # the lower boundary
        ...
        ax.autoscale_view('tight')
        # wrap(ax.get_xlabel())
        ax.set_xlabel(wrap(ax.get_xlabel()), fontsize=7, rotation=90, va='center', ha='left',
                      labelpad=len(ax.get_xlabel()) + 25)  # , fontweight='bold'
        # ax.set_xticks([])
        ax.get_xaxis().set_tick_params(which='both', labelsize=5, top='off', direction='out')
        ax.get_yaxis().set_tick_params(which='both', labelsize=5, right='off', direction='out')
        # ax.set_color('yellow')
